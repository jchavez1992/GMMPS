    ...
    register Button *butPtr;	/* Information about button. */
{
	...
	//return TkCopyAndGlobalEval(butPtr->interp, butPtr->command);
        ...
        // Create a Tcl_Obj for the command
        Tcl_Obj *cmdObj = Tcl_NewStringObj(butPtr->command, -1);
        Tcl_IncrRefCount(cmdObj);

        // Evaluate in global scope
        int result = Tcl_EvalObjEx(butPtr->interp, cmdObj, TCL_EVAL_GLOBAL);

        Tcl_DecrRefCount(cmdObj);                       // Cleanup
        return result;
    }
    return TCL_OK;
}
